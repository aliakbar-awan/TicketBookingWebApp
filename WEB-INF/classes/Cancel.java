	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)throws IOException,ServletException,NumberFormatException
	{
		...
		else
		{
			...
			if(type.equals("customer") && sessID.equals(session.getId()))
			{
				...
				String arr = request.getParameter("seats");
				...
				try
				{
					...
					{
						seat[i] = Integer.parseInt(seatsInString[i]);		//converting seat no.s from string to int				
						String query = "update schedule1 set Seat"+seat[i]+"=0 where JourneyID="+journey+"";
						int s = st.executeUpdate(query);
						count = count + s;
						
					}
						out.println("<h3> Updating data.....   </h3>");
					String query = "delete from tickets where JourneyID="+journey+" AND Seat_num="+arr;
					int s = st.executeUpdate(query);
					count = count + s;
					
					if(count==totalSeats+1)
					{
						out.println("<h3 style='color:#4CAF50'> Your ticket is cancelled successfully! </h3>");
					}
					else
					{
