		...
		else
		{
			...
			if(type.equals("admin"))
			{
				...
				try
				{
					...
					String query = "select *,CAST(CURTIME() AS CHAR) AS curtime1 from schedule1 where Departure='" + from + "'and Destination='"+ to +"';";
					...
					if(rs.first()==false)
					{
						...
						out.print("<h3> There is no bus available for this route. </h3><br>");
					}
					else
					{
						...
						do
						{
							...
							if(departureDate.after(curDate))
							{
								...
								out.print("<form name=booking action='ChangeSchedule' method= post>");
								out.print("<input type=hidden name='journey' value='"+rs.getString("JourneyID")+"'>");
								out.print("<input type=hidden name='depDate' value='"+rs.getString("Departure_date")+"'>");
								out.print("<input type=hidden name='depTime' value='"+rs.getString("Departure_time")+"'>");
